...
offset = 25 #minimal distance between pixel
...
while True:
    ...
    #ret, grey = cv2.threshold(grey,200,255,cv2.THRESH_BINARY_INV)
    blur = cv2.GaussianBlur(grey,(5,5),1)
    #_, thresh = cv2.threshold(blur, 150, 200, cv2.THRESH_BINARY)
    ...
    cv2.line(frame1, (0, count_line_pos), (2000, count_line_pos), (0,255,0), 3)
    ...
    for(i,c) in enumerate(contour):
        ...
        for (x, y) in detect:
            if y < (count_line_pos+offset) and y > (count_line_pos-offset):
                cyl += 1
                cv2.line(frame1, (0, count_line_pos),(2000, count_line_pos), (0,127,255), 3)
                detect.remove((x, y))
